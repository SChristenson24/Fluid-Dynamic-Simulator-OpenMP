...
// Moved indx and combined offset to be able to collapse loops
#pragma omp parallel for collapse(3)
    for (int i = 0; i < ni; ++i)
    {
        for (int j = 0; j < nj; ++j)
        {
            for (int k = 0; k < nk; ++k)
            {
                // Calculate indices and positions within the nested loops to allow collapse
                float dx = (1.0 / ni) * L;
                ...
                int indx = kstart + i * iskip + j * jskip + k;

                // 3-D Taylor-Green vortex
                u[indx] = coef * sin(x / l) * cos(y / l) * cos(z / l);
                v[indx] = -coef * cos(x / l) * sin(y / l) * cos(z / l);
                p[indx] = coef * coef * (cos(2.0 * x / l) + cos(2.0 * y / l)) * (cos(2.0 * z / l) + 2.0) / 16.0;
                w[indx] = 0.0;
            }
        }
    }
...
// Extended offset into indx and moved indx to collapse two loops
    ...
    for (int k = 0; k < nk; ++k)
    {
        for (int j = 0; j < nj; ++j)
        {
            for (int i = 0; i < ni + 1; ++i)
            {
                ...
                pflux *= area; // potential issue with the data dep within the i
    ...
    for (int i = 0; i < ni; ++i)
    {
        for (int k = 0; k < nk + 1; ++k)
        {
            for (int j = 0; j < nj; ++j)
            {
                ...
                float wflux = ((1. / 3.) * (wl + wr) * udotn1 -
                               (1. / 24.) * ((wl + wrr) * udotn2 + (wll + wr) * udotn3) + pterm);

                // Add in viscous fluxes integrate over face area
                pflux *= area; // k dependencies
                uflux = area * (uflux - vcoef * ((5. / 4.) * (ur - ul) - (1. / 12.) * (urr - ull)));
                vflux = area * (vflux - vcoef * ((5. / 4.) * (vr - vl) - (1. / 12.) * (vrr - vll)));
                wflux = area * (wflux - vcoef * ((5. / 4.) * (wr - wl) - (1. / 12.) * (wrr - wll)));
                            ...
                            int iskip, int jskip)
{
    const int kskip = 1;
    float minDt = 1e30;
#pragma omp parallel for collapse(3) reduction(min : minDt)
    for (int i = 0; i < ni; ++i)
    {
        for (int j = 0; j < nj; ++j)
        {
            for (int k = 0; k < nk; ++k)
            {
                const int indx = kstart + i * iskip + j * jskip + k;
                // inviscid timestep
...
#pragma omp parallel for collapse(3)
    for (int i = 0; i < ni; ++i)
    {
        for (int j = 0; j < nj; ++j)
        {
            ...
            {
                const int indx = k + kstart + i * iskip + j * jskip;
                uout[indx] = w1 * u1[indx] + w2 * u2[indx] + w3 * uout[indx]; // same thread throughout computation here
            }
        }
    }
}
